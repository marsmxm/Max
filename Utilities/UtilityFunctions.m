...
// AppleScript constants formerly from Carbon/OpenScripting
...
GenerateTemporaryFilename(NSString *directory, NSString *extension)
{
    ...
    // Get the current process id for constructing the pathname
    pid = getpid();

    // Build the pathname
    // Should look like [directory]/[applicationName]-[pid]-[XXXXXXXX].[extension]
    pathString    = [NSString stringWithFormat:@"%@/%@-%.8x-XXXXXXXX.%@", directory, @"Max", pid, extension];
